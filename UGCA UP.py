...
parser.add_argument('--lambda_cluster', type=float, default=0.003, help='Weight for clustering (center) loss')
...
for iDataSet in range(nDataSet):
    ...
    UGCA_K = 2
    UGCA_TOP_M = 4
    UGCA_UPDATE_INTERVAL = 20
    UGCA_MIN_SRC = 12
    UGCA_MIN_TAR = 12
    UGCA_WARMUP_EPISODES = 500
    ...
    for episode in range(10000):  # EPISODE = 90000
        ...
        if episode % 2 == 0:
            ...
            # [Houston-style] Center loss on source query features (uses true labels)
            ...
            if (episode >= UGCA_WARMUP_EPISODES) and (episode % UGCA_UPDATE_INTERVAL == 0) and (
                    len(selected_classes) > 0):
            ...
            loss = f_loss + shallow_res1_domain_loss + shallow_res2_domain_loss + deep_domain_loss + (
                    args.lambda_cluster * cluster_loss)  # 0.01
            ...
            # Scale CenterLoss gradients (because we multiplied the loss by lambda_cluster)
